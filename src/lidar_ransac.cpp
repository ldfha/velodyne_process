...
void floor_callback(const sensor_msgs::PointCloud2ConstPtr& input)
{
    // 포인트 클라우드 포인터 초기화
...
void obstacle_callback(const sensor_msgs::PointCloud2ConstPtr& input)
{
    // 포인트 클라우드 포인터 초기화
    pcl::PointCloud<pcl::PointXYZI>::Ptr inlierPoints (new pcl::PointCloud<pcl::PointXYZI>),
                                        inlierPoints_neg (new pcl::PointCloud<pcl::PointXYZI>);
...
int main(int argc, char** argv)
{
    ...
    // Subscriber 설정
    ros::Subscriber sub1 = nh.subscribe("lidar_floor_roi", 1, floor_callback);
    ros::Subscriber sub2 = nh.subscribe("lidar_obstacle_roi", 1, obstacle_callback);
