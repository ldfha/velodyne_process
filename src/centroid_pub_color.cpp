...
public:
    ...
    void processCentroids() {
        ...
        if (lidar_centroids_.centroids.empty()) {
            ...
            // Clear lidar centroids after processing
            lidar_centroids_.centroids.clear();
        }

        // Publish annotated images
        if (!annotated_image_left.empty() && !annotated_image_right.empty()) {
            sensor_msgs::ImagePtr annotated_msg_left = cv_bridge::CvImage(left_image_header_, "bgr8", annotated_image_left).toImageMsg();
            sensor_msgs::ImagePtr annotated_msg_right = cv_bridge::CvImage(right_image_header_, "bgr8", annotated_image_right).toImageMsg();
            annotated_image_pub_left_.publish(annotated_msg_left);
            annotated_image_pub_right_.publish(annotated_msg_right);
            ROS_INFO("Published annotated images.");
        } else {
            ROS_WARN("Annotated images are empty, not publishing.");
        }
