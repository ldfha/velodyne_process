...
void filtering_callback(const sensor_msgs::PointCloud2ConstPtr& input)
{
    // 포인트 클라우드 포인터 초기화
...
{
    ros::init(argc, argv, "lane_filter");
    ros::NodeHandle nh;

    // Subscriber 설정
    ros::Subscriber sub = nh.subscribe("ground_point", 1, filtering_callback);

    // Publisher 설정
    pub_lane_marker = nh.advertise<sensor_msgs::PointCloud2>("filtered_lane", 1);

    std::cout << "lane filtering node is running..." << std::endl;
